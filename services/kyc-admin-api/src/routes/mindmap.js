...
function buildMockMindmapTree(sourceText, rootTopic) {
  ...
  const rootLabel =
    normalizeMockText(rootTopic || seedText || "Chủ đề chính").slice(0, 52) || "Chủ đề chính";
...
router.post("/", async (req, res) => {
  try {
    ...
    const extra =
      typeof rootTopic === "string" && rootTopic.trim()
        ? `\nChủ đề gợi ý cho node gốc: ${rootTopic.trim()}`
        : "";
